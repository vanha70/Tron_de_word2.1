"""
PHẦN MỀM TRỘN ĐỀ - THPT MINH ĐỨC (FINAL VERSION - SINGLE ZIP)
Cập nhật:
1. Giao diện: Tên trường in hoa đậm, cập nhật SĐT GV.
2. Logic: P3 trích xuất Key chuẩn xác.
3. Output: Nén Đề + Đáp án vào 1 file Zip duy nhất.
"""
...
# ==================== 1. CẤU HÌNH TRANG ====================
st.set_page_config(
    page_title="Phần mềm Trộn Đề - THPT Minh Đức",
    page_icon="📚",
    layout="centered",
...
def add_header_to_doc(doc, body, exam_code):
    """Thêm Header Trường và Mã đề vào đầu file"""
    nodes = []
    # Dòng 1: Tên Trường
    ...
    # Chèn ngược để giữ thứ tự ở đầu
    ...
    if len(indices) >= 2:
        ...
        for opt in opts:
            ...
            for r in runs:
                ...
                remove_answer_signal(r) # Xóa dấu
...
def process_short(q_blocks):
    """Xử lý Phần 3 (Key)"""
    key_val = ""
    # Quét tất cả các block của câu hỏi này để tìm key
    for b in q_blocks:
        ...
        for t in t_nodes:
            if t.firstChild:
                ...
                if m:
                    key_val = m.group(1) # Lấy giá trị key
...
def generate_mix(file_bytes, num_copies):
    # Buffer cho file ZIP tổng (chứa đề + đáp án)
